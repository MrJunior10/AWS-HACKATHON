...
class StudyAssistantHandler:
    ...
    def generate_quiz(self, roadmap_content="", difficulty_level="intermediate", focus_areas="general", num_questions=10) -> str:
        """Indestructible Quiz Generator"""
        # 1. Protect against the NoneType Crash
        safe_roadmap = str(roadmap_content) if roadmap_content else f"General knowledge about {self.topic}"
        safe_roadmap = safe_roadmap[:3000] # Safe to slice now!
        
        # 2. Hardcoded prompt bypassing the broken YAML entirely
        hardcoded_prompt = f"""
        You are a strict backend API. Generate exactly {num_questions} multiple-choice questions about: {self.topic}.
        Difficulty: {difficulty_level}. Focus: {focus_areas}.
        
        CONTEXT TO BASE QUESTIONS ON:
        {safe_roadmap}
        
        CRITICAL FATAL ERROR WARNING: 
        You MUST output ONLY a raw JSON array. NO markdown blocks. NO backticks. NO conversational text.
        ...
        try:
            ...
            resp = generator.run(hardcoded_prompt, stream=False)
            output = str(resp.content).strip()
            
            # 3. Clean up AI markdown garbage so the frontend doesn't choke
            ...
            return "[]" # Fails safely, triggers frontend fallback, keeps server alive
