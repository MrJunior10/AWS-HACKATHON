...
async function createLobby() {
    ...
    // HACKATHON TRICK: Scrape the roadmap text right off the UI!
    const roadmapText = document.getElementById('roadmap-content').innerText || "General knowledge on " + activeTopic;

    try {
        document.getElementById('arena-setup').innerHTML = `<p class="text-white mt-10">Generating 10 Battle Questions...</p>`;
        ...
        try {
            ...
            let parsedQuiz = data.quiz || data.quiz_data;
            if (typeof parsedQuiz === 'string') parsedQuiz = JSON.parse(parsedQuiz);
            ...
            console.error("Arena Quiz Error:", e);
            quizData = getBulletproofQuiz(activeTopic);
        }
